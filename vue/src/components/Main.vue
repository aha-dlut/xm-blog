<template>
  <div>
    ...
    <div style="padding: 15px">
      <a style="color: #666" :href="'/front/diskHome?category=' + category">全部文件 <i class="el-icon-arrow-right" v-if="folders.length"></i></a>
      <a style="color: #666" :href="'/front/diskHome?category=' + category + '&folderId=' + item.id" v-for="(item, index) in folders" :key="item.id">
...
export default {
  ...
  methods: {
    ...
    openFile(row) {
      if (row.folder === '是') { // 文件夹
        location.href = '/front/diskHome?category=' + this.category + '&folderId=' + row.id
      } else {
